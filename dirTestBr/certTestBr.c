...
int recebeDiretorio(char* diretorio){
	...
	if((dirBrasil = opendir(nomeDirBrasil)) == NULL){
                printf("Erro ao acessar diretorio brasil\n");
		return 1;
	}
	...
	int numNosBr = 0;
	while((arqbr = readdir(dirBrasil)) != NULL){
		if(strcmp(arqbr->d_name,"..") && strcmp(arqbr->d_name,".")){
			numNosBr++;
		}
	}
	...
	t_certificate *certificadosVetorBr = (t_certificate*) calloc(numNosBr,sizeof(t_certificate));
	if(certificadosVetorBr == NULL){
		printf("erro no calloc\n");
	...
	while((arqbr = readdir(dirBrasil)) != NULL){
		if(strcmp(arqbr->d_name,"..") && strcmp(arqbr->d_name,".")){
			strcpy(certificadosVetorBr[i].filename,arqbr->d_name);
			strcpy(straux,nomeDirBrasil);
			strcat(straux,arqbr->d_name);
			readFile(straux,stringModulo);
			strcpy(certificadosVetorBr[i].module,stringModulo);
			i++;
			if(setDebug){
				printf("%s\n",stringModulo);
			}
	...
	int tamModA;
	int tamModResult;
	for(i=0;i<numNosBr;i++){
            if(!setDebug){
                system("clear");
            }
            printf("Progresso: %.2f%%\n",((float)i/(float)numNosBr) * 100);
		for(j = 0;j<numNos;j++){
			//fazer mdc entre i e j
			strcpy(moduloA,certificadosVetorBr[i].module);
			strcpy(moduloB,certificadosVetor[j].module);
			if(setDebug){
				printf("mdc entre\n%s e %s\n",certificadosVetorBr[i].filename,certificadosVetor[j].filename);
			}
			...
			if(!strcmp(mdcResultado,"1")){
				...
				fprintf(ptResultado,"***********************************\nMDC\n%s  x  %s\nModulos iguais\n%s\n\n",certificadosVetorBr[i].filename,certificadosVetor[j].filename,mdcResultado);
			}else{
				fprintf(ptResultado,ANSI_COLOR_RED"***********************************\nMDC\n%s  x  %s\nPrimo em comum\n%s\n\n"ANSI_COLOR_RESET,certificadosVetorBr[i].filename,certificadosVetor[j].filename,mdcResultado);
			}
...
int main(int argc, char** argv){
	...
	if(argc != 4){
		printf("Deve-se passar 3 argumentos:\n./programa <pasta contendo certificados> <nomedorelatorio> <pasta de certs comparativos> \n");
		return 1;
	}else{
                strcpy(nomeDirBrasil,argv[3]);
